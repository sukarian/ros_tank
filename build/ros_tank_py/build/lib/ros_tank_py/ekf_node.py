...
class ExtendedKalmanFilter:
    def __init__(self):
        ...
        self.Q = np.diag([0.01, 0.01, 0.01, 0.1, 0.1, 0.1, 0.5, 0.5, 0.1, 0.1])
    ...
    def predict(self, dt):
        ...
        # Add gravity compensation to accelerations in global frame
        # Convert body frame accelerations to global frame
        cos_theta = np.cos(theta)
        sin_theta = np.sin(theta)
        
        # Transform body accelerations to global frame
        ax_global = ax * cos_theta - ay * sin_theta
        ay_global = ax * sin_theta + ay * cos_theta
        
        # Apply gravity compensation
        self.state[3] += (ax_global) * dt  # vx update with global ax
        self.state[4] += (ay_global + self.g) * dt  # vy update with global ay + gravity
        
        # Update position with corrected velocities
    ...
    def update_imu(self, linear_acceleration, angular_velocity, imu_bias):
        ...
        expected_ax = self.state[6] - self.g * np.sin(pitch)
        expected_ay = self.state[7] + self.g * np.cos(pitch) * np.sin(roll)
        ...
        # Remove bias
        z_corrected = z - np.array(imu_bias)
