		...
		// Create timer (10ms period)
        	write_timer_ = this->create_wall_timer(
         		std::chrono::milliseconds(100),
            		std::bind(&MotorControlNode::write_callback, this));

                read_timer_ = this->create_wall_timer(
                        std::chrono::milliseconds(100),
                        std::bind(&MotorControlNode::read_callback, this));
...
private:
    void configure_serial_port()
    {
        ...
        tty.c_oflag &= ~ONLCR; // Prevent conversion of newline to carriage ret>

        // Set timeout - return immediately with available data
	...
	void write_callback(){
		char buffer[32];
		float left_cmd = this->left_motor_cmd_.data;
		float right_cmd = this->right_motor_cmd_.data;
		sprintf(buffer, "%.3f %.3f\n", left_cmd, right_cmd);
		RCLCPP_INFO(this->get_logger(), "Serial MSG to arduino: %s\n", buffer);
		if (serial_fd_ != -1) {
       			ssize_t bytes_written = write(serial_fd_, buffer, strlen(buffer));
        		tcdrain(serial_fd_);
			if (bytes_written == -1) {
